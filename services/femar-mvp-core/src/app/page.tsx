...
export default function Home() {
  ...
  return (
    <ClientErrorBoundary fallbackTitle="Centro de Control (Modo Recuperación)">
              ...
              <GlassWidget title={t("hardware")} icon={MonitorSmartphone}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {devices.length === 0 ? (
                    ...
                    const isOnline = device.lastSync && (new Date().getTime() - new Date(device.lastSync).getTime() < 300000); // 5 mins
                    return (
                      <div key={device.id} className="p-4 rounded-xl bg-zinc-800/40 border border-zinc-700/50 flex flex-col gap-2 relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-zinc-700/50">
                          <span className="text-xs text-zinc-400">
                            {device.lastSync ? new Date(device.lastSync).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'N/A'}
                          </span>
