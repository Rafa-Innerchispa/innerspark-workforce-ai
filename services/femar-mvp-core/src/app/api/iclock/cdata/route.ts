...
export async function POST(req: NextRequest) {
  try {
    ...
    // Tenant ownership comes from the registered device. An unassigned device
    // may upload raw events, but it must never influence another tenant's payroll.
    const deviceSnapshot = await db.collection('devices').doc(sn).get();
    const deviceCompanyId = deviceSnapshot.exists
      ? String(deviceSnapshot.data()?.companyId || deviceSnapshot.data()?.tenantId || '')
      : '';
    ...
    if (table === 'ATTLOG') {
      ...
      for (const line of lines) {
        ...
        if (parts.length >= 2) {
          ...
          batch.set(docRef, {
            serial_number: sn,
            companyId: deviceCompanyId || null,
            tenantId: deviceCompanyId || null,
            user_id: parts[0],
            timestamp: parts[1],
            state: parts[2] || '',
            type: parts[3] || '',
          ...
          if (deviceCompanyId) {
            processCheckinNovelty(parts[0], parts[1], 'ADMS', deviceCompanyId)
              .catch((error) => console.error('Novelty processing error:', error));
          } else {
            console.warn(`Skipping novelty for unassigned device ${sn}: companyId/tenantId required`);
          }
        }
      }
