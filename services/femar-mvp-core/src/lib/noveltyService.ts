...
async function loadSchedule(companyId: string, employeeId: string, dateKey: string): Promise<ScheduleRecord | null> {
  // Query by tenant first so a caller can never read another tenant's schedule.
  // We intentionally filter employee/date in-process to avoid requiring a new composite index during the hackathon.
