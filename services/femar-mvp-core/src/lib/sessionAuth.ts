...
export async function requireJudgeConsoleAccess(): Promise<SessionUser | NextResponse> {
  const user = await requireSession();
  if (user instanceof NextResponse) return user;
  const cid = user.companyId.toLowerCase();
  if (user.role === 'superadmin' || cid === 'hackathon' || cid === 'pcdoctor' || cid === 'ent_pcdoctor') {
    return user;
  }
  return forbiddenResponse('Judge Console access denied');
}
