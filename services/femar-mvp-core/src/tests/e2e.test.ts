...
jest.mock('next/server', () => {
  class MockNextResponse {
    ...
    constructor(body: unknown, init: { status?: number } | undefined) {
      ...
      (this as { text?: () => Promise<unknown> }).text = async () => body;
    }
...
describe('E2E Endpoints', () => {
  ...
  describe('ADMS CData Checkin (/api/iclock/cdata)', () => {
    ...
    it('should write a synthetic checkin and invoke novelty processing', async () => {
      ...
      expect(db.collection).toHaveBeenCalledWith('novelties');
