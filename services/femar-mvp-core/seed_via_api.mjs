...
async function run() {
  const targetPort = 3002; // Dev server port
  const baseUrl = `http://localhost:${targetPort}/api/employees`;
  
  console.log(`Seeding database via API on: ${baseUrl}`);
  const companies = ["femar", "innerspark-labs"];
  
  for (const companyId of companies) {
    console.log(`Sending 50 employees for company: ${companyId}`);
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      try {
        ...
        if (res.ok) {
          console.log(`[OK] Created ${name} (${cedula})`);
        } else {
          console.error(`[ERROR] Failed for ${name}:`, await res.text());
        }
      } catch (err) {
